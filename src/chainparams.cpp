...
public:
    CMainParams() {
        ...
        /* The value of ~38 ROD is calculated to yield the desired total
...
public:
    CTestNetParams() {
        ...
        consensus.defaultAssumeValid = uint256S(""); // 110'000
...
public:
    explicit SigNetParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock (1642373999, 534547, 0x1e0ffff0,
                                      pszTimestampTestnet,
                                      uint160S (hexPremineAddressMainnet));
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x01"));
